...
> A clean, offline-first Android money manager built with **Expo** and **React Native + TypeScript**.
> Google Sign-In for identity. Google Drive for backup. GitHub Actions for APK delivery.
> No Firebase. APK distribution is handled through GitHub Releases. No emojis inside the app.
...
- `GITHUB_TOKEN` for creating releases is provided automatically by GitHub Actions — you do not add it manually
...
GITHUB_TOKEN            → auto-provided by GitHub, used to create Releases and upload the APK
```
...
### Notification handler (set at module level)
...
- [ ] Add `.gitignore` entries for `.env`, `android/`, `ios/`, `*.apk`, `*.aab`
...
*CashWise — Built for real personal finance tracking. No subscriptions. No ads. Your data stays on your device and your own Google Drive.*
